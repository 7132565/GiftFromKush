...
bool deleteAnywhere(int position){//we used the bool return type just for making a better UI
struct Node* temp=head;
...
if(position!=1){
        int i;
        for(i=1;i<position-1;i++){
            temp=temp->next;//temp points to the n-1th node
        }
        struct Node* temp1=temp->next;//now temp1 points to the nth node
        temp=temp1->next;//finally temp points to the n+1th node
        free(temp1);//previous node that is nth node is deleted
        return true;
    }
